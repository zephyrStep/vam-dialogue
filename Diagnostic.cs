...
namespace VamDialogue
{
    ...
    public class Diagnostics : MVRScript
    {
        ...
        {
            CreateButton("Get Hierarchy").button.onClick.AddListener(() =>
                SuperController.LogMessage(GetHierarchy(containingAtom.gameObject)));
            ...
            HashSet<GameObject> found)
        {
            if (found.Contains(o))
            {
                SuperController.LogMessage(
                    "|" + new String(' ', indent) + " [" + o.tag + "] " + o.name + " {RECURSIVE}");
                return;
            }
            ...
            SuperController.LogMessage(
                ...
                " -> " +
                (showScripts
                    ? string.Join(", ", o.GetComponents<MonoBehaviour>().Select(b => b.ToString()).ToArray())
                    : "")
            );
        ...
        {
            return gameObject.GetComponentsInChildren<MonoBehaviour>().GroupBy(b => b.gameObject).Select(x => x.Key)
